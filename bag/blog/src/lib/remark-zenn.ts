...
/**
 * Remark plugin to handle custom markdown syntax
 */
export const remarkZenn: Plugin<[], Root> = () => {
    return (tree) => {
        // 1. Handle Container Directives (:::message, :::details)
        visit(tree, 'containerDirective' as any, (node: CustomNode) => {
            ...
            if (name === 'message') {
                node.data = {
                    ...
                    hProperties: {
                        ...
                        className: `zenn-message zenn-message-${attributes?.type || 'info'}`,
                    },
                };
        ...
        visit(tree, 'leafDirective' as any, (node: CustomNode) => {
            const { name, attributes } = node;

            if (['youtube', 'twitter', 'github', 'gist', 'codepen', 'slideshare', 'speakerdeck', 'docswell', 'jsfiddle', 'codesandbox', 'stackblitz', 'figma'].includes(name || '')) {
                node.data = {
                    hName: name,
                    hProperties: {
