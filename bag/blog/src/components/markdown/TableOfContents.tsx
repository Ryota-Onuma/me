...
export const TableOfContents = ({ content }: TableOfContentsProps): React.ReactNode => {
    ...
    return (
        <nav className="mb-12 p-6 bg-white border border-black/10 rounded-lg">
            <p className="text-xs font-medium mb-4 text-black/45">Contents</p>
            <ul className="space-y-3">
                {headings.map(h => (
                    <li key={h.id}>
                        <a
                            href={`#${h.id}`}
                            className="text-sm font-medium text-black/60 hover:text-black transition-colors flex items-center gap-2 group"
                        >
