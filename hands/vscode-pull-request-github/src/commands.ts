...
import { AutomatedReviewService, REVIEW_ASSISTANT_CONFIG_NAMESPACE, ReviewAgentKind } from './github/automatedReviewService';
...
export function registerCommands(
	...
	const automatedReviewService = new AutomatedReviewService(telemetry);
	context.subscriptions.push(automatedReviewService);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('pr.runAutomatedReview', async (arg?: PRNode | PullRequestModel) => {
			try {
				const target = await resolvePullRequestForAutomatedReview(arg, reposManager);
				const provider = await pickAutomatedReviewProvider();
				if (!provider) {
					return;
				}
				const label = getAutomatedReviewProviderLabel(provider);
				await vscode.window.withProgress({
					location: vscode.ProgressLocation.Notification,
					title: vscode.l10n.t('{0} review for pull request #{1}', label, target.pullRequest.number),
					cancellable: true
				}, async (_progress, token) => {
					await automatedReviewService.run(provider, target.pullRequest, target.folderManager, token);
				});
				vscode.window.showInformationMessage(vscode.l10n.t('{0} review finished for pull request #{1}. Check the "Pull Request Automated Review" output.', label, target.pullRequest.number));
			} catch (error) {
...
interface AutomatedReviewTarget {
	pullRequest: PullRequestModel;
	folderManager: FolderRepositoryManager;
}

async function resolvePullRequestForAutomatedReview(arg: PRNode | PullRequestModel | undefined, reposManager: RepositoriesManager): Promise<AutomatedReviewTarget> {
	if (arg instanceof PRNode) {
		return {
			pullRequest: arg.pullRequestModel,
			folderManager: ensureFolderManagerForPullRequest(arg.pullRequestModel, reposManager)
		};
	}

	if (arg instanceof PullRequestModel) {
		return {
			pullRequest: arg,
			folderManager: ensureFolderManagerForPullRequest(arg, reposManager)
		};
	}

	const active = reposManager.folderManagers
		.map(folderManager => folderManager.activePullRequest ? { pullRequest: folderManager.activePullRequest, folderManager } : undefined)
		.filter((value): value is AutomatedReviewTarget => !!value);

	if (!active.length) {
		throw new Error(vscode.l10n.t('No active pull request available for automated review.'));
	}
	...
	const items: (vscode.QuickPickItem & { target: AutomatedReviewTarget })[] = active.map(target => ({
		label: `#${target.pullRequest.number} ${target.pullRequest.title}`,
		description: target.folderManager.repository.rootUri.fsPath,
		detail: target.pullRequest.html_url,
		target
	}));

	const selection = await vscode.window.showQuickPick(items, {
		placeHolder: vscode.l10n.t('Select a pull request to review automatically'),
		ignoreFocusOut: true
	});
...
function ensureFolderManagerForPullRequest(pullRequest: PullRequestModel, reposManager: RepositoriesManager): FolderRepositoryManager {
	const folderManager = reposManager.getManagerForIssueModel(pullRequest);
	if (!folderManager) {
		throw new Error(vscode.l10n.t('Unable to resolve repository for pull request #{0}.', pullRequest.number));
	}
	return folderManager;
}

function getAutomatedReviewProviderLabel(provider: ReviewAgentKind): string {
	return provider === ReviewAgentKind.Codex
		? vscode.l10n.t('Codex CLI')
		: vscode.l10n.t('Claude Code');
}

function getConfiguredCommandSummary(provider: ReviewAgentKind, config: vscode.WorkspaceConfiguration): string {
	...
	return argLine.length ? `${command} ${argLine}` : command;
}

async function pickAutomatedReviewProvider(): Promise<ReviewAgentKind | undefined> {
	const config = vscode.workspace.getConfiguration(REVIEW_ASSISTANT_CONFIG_NAMESPACE);
	const defaultProvider = config.get<ReviewAgentKind>('defaultProvider', ReviewAgentKind.Codex);
	const prompt = config.get<boolean>('promptForProvider', true);
	if (!prompt) {
		return defaultProvider;
	}

	const createItem = (provider: ReviewAgentKind) => ({
		label: getAutomatedReviewProviderLabel(provider),
		description: getConfiguredCommandSummary(provider, config),
		picked: provider === defaultProvider,
		provider
	});

	const selection = await vscode.window.showQuickPick([
		createItem(ReviewAgentKind.Codex),
		createItem(ReviewAgentKind.Claude)
	] as (vscode.QuickPickItem & { provider: ReviewAgentKind })[], {
		placeHolder: vscode.l10n.t('Select an automated review provider'),
		ignoreFocusOut: true
	});
