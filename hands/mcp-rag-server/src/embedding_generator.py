...
try:
    # 重い依存を持つため、存在しない環境でもテストできるよう例外を許容
    from sentence_transformers import SentenceTransformer  # type: ignore
except Exception:  # ImportError を含む広めの例外で安全側

    class SentenceTransformer:  # type: ignore
        """テスト用の軽量スタブ。実行時にはモックで置き換えられる想定。"""

        def __init__(self, *_, **__):
            raise RuntimeError(
                "SentenceTransformer がインストールされていません。実運用では 'sentence-transformers' をインストールしてください。"
            )


# .envは使用せず、JSON由来のENVまたは既定値を使用
